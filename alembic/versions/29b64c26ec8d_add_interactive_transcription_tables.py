...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_transcription_segments_usage_id'), 'transcription_segments', ['usage_id'], unique=False)
    # Add unique constraint for (usage_id, segment_index)
    with op.batch_alter_table('transcription_segments', schema=None) as batch_op:
        batch_op.create_unique_constraint('uq_transcription_segments_usage_segment', ['usage_id', 'segment_index'])

    op.create_table('transcription_states',
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('usage_id', sa.Integer(), nullable=False),
    ...
    op.create_index(op.f('ix_transcription_states_usage_id'), 'transcription_states', ['usage_id'], unique=False)
    ...
    with op.batch_alter_table('transcription_states', schema=None) as batch_op:
        batch_op.create_unique_constraint('uq_transcription_states_message_chat', ['message_id', 'chat_id'])
        batch_op.create_index('ix_transcription_states_message_chat', ['message_id', 'chat_id'])

    op.create_table('transcription_variants',
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('usage_id', sa.Integer(), nullable=False),
    ...
    op.create_index(op.f('ix_transcription_variants_usage_id'), 'transcription_variants', ['usage_id'], unique=False)
