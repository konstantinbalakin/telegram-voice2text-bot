...
def upgrade() -> None:
    ...
    with op.batch_alter_table("usage", schema=None) as batch_op:
        ...
        batch_op.alter_column("voice_duration_seconds", existing_type=sa.Integer(), nullable=True)
        batch_op.alter_column("model_size", existing_type=sa.String(length=50), nullable=True)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("usage", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("transcription_text", sa.Text(), nullable=False, server_default="")
        )

        # Make columns non-nullable again
        batch_op.alter_column("model_size", existing_type=sa.String(length=50), nullable=False)
        batch_op.alter_column("voice_duration_seconds", existing_type=sa.Integer(), nullable=False)
