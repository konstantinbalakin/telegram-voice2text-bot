...
Unit-тесты НИКОГДА не должны зависеть от локального окружения:

- **`get_session()`** — всегда мокать через `@patch("src.module.get_session")`. Если вызываемый метод внутри себя обращается к БД — мок нужен даже если тест проверяет другую логику.
- **`settings.*`** — патчить ВСЕ флаги, влияющие на ветвление: `billing_enabled`, `billing_test_mode`, `interactive_mode_enabled` и т.д. Нельзя полагаться на дефолтные значения, `.env` их перезаписывает.
- **Правило**: если тест проходит локально, но может упасть на чистой машине без `.env` и без SQLite файла — тест написан неправильно.
